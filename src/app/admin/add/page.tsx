...
import { ArrowLeft, Upload, Image as ImageIcon, X, Plus, GripVertical } from "lucide-react";
...
export default function AddService() {
  ...
  const handleFileUpload = async (file: File) => {
    ...
    // Show loading state implicitly or via toast (simplified here)
    ...
    <div>
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          ...
          <p className="text-white/40 text-sm">Create a new entertainment service</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 max-w-4xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-2.5 rounded-lg bg-dark-700 border border-white/10 text-white text-sm placeholder-white/20 focus:border-gold/50 outline-none transition-all"
                placeholder="nashik-dhol"
              />
              <p className="text-white/20 text-xs mt-1">/services/{formData.slug}</p>
            </div>
            <div>
            ...
            <div className="md:col-span-2">
              ...
              <textarea
                ...
                className="w-full px-4 py-2.5 rounded-lg bg-dark-700 border border-white/10 text-white text-sm placeholder-white/20 focus:border-gold/50 outline-none transition-all resize-none"
                rows={2}
            ...
            <div className="space-y-4">
              ...
              >
                ...
                <p className="text-white/40 text-sm">
                  Drag & drop an image or <span className="text-gold">browse</span>
                </p>
              ...
              <div key={index} className="flex gap-2">
                ...
                {features.length > 1 && (
                  <button
                    ...
                    className="p-2.5 text-white/20 hover:text-red-400 hover:bg-red-400/10 rounded-lg transition-all"
                  >
                    <X size={14} />
                  </button>
                )}
              ...
              <div key={pkgIndex} className="rounded-lg border border-white/5 bg-dark-700/50 p-4">
                ...
                <div>
                  ...
                  <div className="space-y-1.5">
                    {pkg.items.map((item, itemIndex) => (
                      <div key={itemIndex} className="flex gap-2">
                        ...
                        {pkg.items.length > 1 && (
                          <button
                            type="button"
                            onClick={() => removePackageItem(pkgIndex, itemIndex)}
                            className="p-2 text-white/20 hover:text-red-400 rounded-lg transition-all"
                          >
                            <X size={12} />
                          </button>
                        )}
                    ...
                    <button
                      type="button"
                      onClick={() => addPackageItem(pkgIndex)}
        ...
        <div className="flex justify-end gap-3 pb-8">
          <Link
            href="/admin"
            className="px-6 py-2.5 border border-white/10 text-white/50 rounded-xl text-sm hover:bg-white/5 transition-all"
          >
            Cancel
          </Link>
          <button
            type="submit"
            className="px-8 py-2.5 bg-gradient-to-r from-gold to-gold-dark text-dark-900 rounded-xl text-sm font-semibold hover:shadow-lg hover:shadow-gold/20 transition-all"
          >
            Add Service
          </button>
        </div>
